...
// Update products to have proper sell_price if they don't have it
echo "Updating product prices...\n";
foreach ($products as $index => $product) {
    if (!$product->sell_price) {
        $price = 100000 + ($index * 50000);
        $product->update(['sell_price' => $price]);
        echo "  Updated {$product->name} price to " . number_format($price) . "\n";
    }
}

// Create a DRAFT quotation for testing updates
echo "\n1. Creating DRAFT quotation for update testing...\n";
$draftQuotation = DB::transaction(function () use ($customers, $products, $salesUser) {
    ...
    $totalPrice = $item1->sell_price * 2; // 2 units
...
$submittedQuotation2 = DB::transaction(function () use ($customers, $products, $salesUser) {
    ...
    $totalPrice = $product->sell_price * 1;
    $totalPriceWithTax = $totalPrice * 1.11;
